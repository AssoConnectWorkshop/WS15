...
import { loadRemoteProgress, saveRemoteProgress, getSession } from "../actions";
...
// Show lead modal after this many article completions
const LEAD_TRIGGER_COUNT = 2;
...
function loadLocalProgress(): Progress {
  ...
  try {
    ...
    if (!raw) return { completedArticles: new Set(), unlockedBadges: new Set(), quizResults: {}, earnedQuizXp: {} };
...
function mergeRemote(local: Progress, remote: { completed_articles: string[]; unlocked_badges: string[]; quiz_results: Record<string, QuizState>; earned_quiz_xp: Record<string, number> } | null): Progress {
  if (!remote) return local;
  return {
    completedArticles: new Set([...local.completedArticles, ...remote.completed_articles]),
    unlockedBadges: new Set([...local.unlockedBadges, ...remote.unlocked_badges]),
    quizResults: { ...remote.quiz_results, ...local.quizResults },
    earnedQuizXp: { ...remote.earned_quiz_xp, ...local.earnedQuizXp },
  };
}
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="h-2.5 overflow-hidden rounded-full bg-[#E8EEFF]">
        <div className="h-full rounded-full asso-gradient transition-all duration-700" style={{ width: `${pct}%` }} />
      </div>
    </div>
  );
}
...
function BadgeCard({ badge, unlocked }: { badge: Parcours["badge"]; unlocked: boolean }) {
  return (
    <div className={`flex flex-col items-center gap-2 rounded-2xl border-2 p-4 text-center transition-all duration-300 w-28 shrink-0 ${
      unlocked ? "border-amber-300 bg-gradient-to-b from-amber-50 to-yellow-50 shadow-lg shadow-amber-100"
      ...
      {unlocked && <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-700">✓ Débloqué</span>}
    </div>
  );
}
...
function ArticleCard({ article, completed, onToggle }: { article: Article; completed: boolean; onToggle: () => void }) {
  ...
  const hasEmbed = article.type === "video" && article.youtubeId && article.youtubeId !== "TODO_REPLACE" && !article.youtubeId?.startsWith("TODO");

  return (
    <div className={`rounded-2xl border-2 p-4 transition-all duration-200 ${
      completed ? "border-[#00C49A]/40 bg-[#E6FBF6]"
      ...
      <div className="flex items-start gap-3">
        ...
        <button
          ...
          className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 transition-all duration-200 ${
            completed ? "border-[#00C49A] bg-[#00C49A] text-white scale-110" : "border-gray-300 hover:border-[#3D5AF1]"
          }`}
        >
          {completed && (
        ...
        <div className="flex-1 min-w-0">
          <div className="mb-1.5 flex flex-wrap items-center gap-2">
            <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${tc.bg}`}>{tc.icon} {tc.label}</span>
          ...
          {hasEmbed ? (
            ...
            /* Fallback: link to channel when ID not yet set */
            <div className="mb-3 flex items-center gap-3 overflow-hidden rounded-xl border-2 border-dashed border-red-200 bg-red-50 px-4 py-3">
              <span className="text-2xl">▶️</span>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-800">{article.title}</p>
                <a href="https://www.youtube.com/@assoconnect" target="_blank" rel="noopener noreferrer"
                   className="mt-0.5 flex items-center gap-1 text-sm font-medium text-red-600 hover:underline">
                  Voir sur la chaîne YouTube AssoConnect
            ...
            <a href={article.url} target="_blank" rel="noopener noreferrer"
               className={`block font-semibold leading-snug transition-colors ${completed ? "text-gray-400 line-through" : "text-gray-900 hover:text-[#3D5AF1]"}`}>
              {article.title}
              {!completed && (
          ...
          {completed && <p className="mt-1.5 text-xs font-semibold text-[#00C49A]">✓ Lu et validé — super boulot !</p>}
        </div>
      </div>
    </div>
  );
...
function QuizCard({ mission, quizState, onSubmit, onReset }: {
  mission: Mission; quizState: QuizState | undefined;
  onSubmit: (answers: Record<string, number>) => void; onReset: () => void;
}) {
  ...
  return (
    <div className="rounded-2xl border-2 border-[#3D5AF1]/20 bg-[#F5F7FF] p-6">
      <div className="mb-5 flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#3D5AF1] text-xl text-white">🧠</div>
        <div className="flex-1">
          <h5 className="font-bold text-gray-900">Quiz de validation</h5>
          <p className="text-sm text-gray-500">
            {submitted
              ? `${score}/${total} bonnes réponses · +${Math.round((score / total) * mission.quizXp)} XP`
              : `${total} questions · jusqu'à +${mission.quizXp} XP bonus`}
          </p>
        </div>
        {submitted && (
          <div className={`rounded-full px-4 py-1.5 text-sm font-bold ${passed ? "bg-[#00C49A] text-white" : "bg-orange-100 text-orange-700"}`}>
          ...
          return (
            <div key={q.id}>
              <p className="mb-3 font-semibold text-gray-900">
                <span className="mr-2 inline-flex h-6 w-6 items-center justify-center rounded-full bg-[#3D5AF1] text-xs font-bold text-white">{qi + 1}</span>
                {q.question}
              </p>
              <div className="space-y-2">
                {q.options.map((opt, idx) => {
                  ...
                  if (!submitted) cls += isChosen ? "border-[#3D5AF1] bg-[#E8EEFF] text-[#3D5AF1]" : "border-gray-200 bg-white hover:border-[#3D5AF1]/40 cursor-pointer";
                  else if (isRight) cls += "border-[#00C49A] bg-[#E6FBF6] text-[#007a60]";
                  ...
                  return (
                    <button key={idx} className={cls} onClick={() => !submitted && setAnswers((p) => ({ ...p, [q.id]: idx }))} disabled={submitted}>
                      ...
                      }`}>{submitted && isRight ? "✓" : submitted && isChosen && !isRight ? "✗" : String.fromCharCode(65 + idx)}</span>
                      {opt}
                    </button>
                  );
                })}
              ...
              {submitted && (
                <div className={`mt-2 flex items-start gap-2 rounded-xl px-3 py-2.5 text-sm ${isCorrect ? "bg-[#E6FBF6] text-[#007a60]" : "bg-blue-50 text-blue-800"}`}>
                  <span>{isCorrect ? "💡" : "📖"}</span><span>{q.explanation}</span>
                </div>
              )}
            </div>
          );
      ...
      <div className="mt-6 flex items-center gap-3">
        {!submitted ? (
          <>
            <button disabled={!allAnswered} onClick={() => onSubmit(answers)}
              className={`rounded-xl px-6 py-3 font-bold text-white transition-all duration-200 ${allAnswered ? "asso-gradient shadow-lg shadow-[#3D5AF1]/25 hover:opacity-90 hover:scale-105" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}>
              Valider mes réponses ✓
            </button>
            <span className="text-sm text-gray-400">{Object.keys(answers).length}/{mission.quiz.length} répondu{Object.keys(answers).length > 1 ? "es" : "e"}</span>
          </>
        ) : (
          <button onClick={() => { setAnswers({}); onReset(); }}
...
function MissionCard({ mission, progress, onToggleArticle, onSubmitQuiz, onResetQuiz }: {
  ...
  return (
    <div className={`overflow-hidden rounded-2xl border-2 transition-all duration-300 ${isDone ? "border-[#00C49A]/50 bg-[#F0FDF9]" : "border-[#E8EEFF] bg-white"}`}>
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center gap-4 p-5 text-left">
        <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl text-sm font-bold transition-all ${isDone ? "bg-[#00C49A] text-white shadow-lg shadow-[#00C49A]/30" : allArticlesDone ? "asso-gradient text-white" : "bg-[#E8EEFF] text-[#3D5AF1]"}`}>
          {isDone ? (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isDone && <span className="rounded-full bg-[#00C49A]/10 px-2.5 py-0.5 text-xs font-bold text-[#007a60]">+{mission.xp + (progress.earnedQuizXp[mission.id] ?? 0)} XP ✓</span>}
            {allArticlesDone && !quizDone && <span className="animate-pulse rounded-full bg-[#3D5AF1]/10 px-2.5 py-0.5 text-xs font-bold text-[#3D5AF1]">🧠 Quiz dispo !</span>}
          </div>
          ...
          <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[#E8EEFF]">
            <div className={`h-full rounded-full transition-all duration-500 ${isDone ? "bg-[#00C49A]" : "asso-gradient"}`} style={{ width: `${pct}%` }} />
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          ...
          <div className={`rounded-full p-1 transition-all ${open ? "bg-[#E8EEFF] rotate-180" : "bg-[#F8F9FF]"}`}>
            <svg className="h-4 w-4 text-[#3D5AF1]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
          </div>
        </div>
      </button>
        ...
        <div className="border-t border-[#E8EEFF] p-5 space-y-4">
          <div className="space-y-3">
            {mission.articles.map((article) => (
              <ArticleCard key={article.id} article={article} completed={progress.completedArticles.has(article.id)} onToggle={() => onToggleArticle(article.id)} />
            ))}
          </div>
          {!allArticlesDone && (
            <div className="flex items-center gap-3 rounded-xl border-2 border-dashed border-[#E8EEFF] bg-[#F8F9FF] px-4 py-3 text-sm text-gray-500">
              <span>🔒</span><span>Lisez tous les contenus pour débloquer le quiz de ce module !</span>
            </div>
          )}
          {allArticlesDone && <QuizCard mission={mission} quizState={progress.quizResults[mission.id]} onSubmit={(a) => onSubmitQuiz(mission.id, a)} onReset={() => onResetQuiz(mission.id)} />}
        </div>
      )}
    </div>
  );
    ...
    <section className="rounded-3xl border-2 border-[#E8EEFF] bg-white p-6 shadow-sm shadow-[#3D5AF1]/5">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-3 text-xs text-gray-400">{completedCount}/{allArticleIds.length} contenus lus</div>
        </div>
        <BadgeCard badge={parcours.badge} unlocked={badgeUnlocked} />
      </div>
      <div className="mb-5"><XPBar current={earnedXP} max={totalXP} /></div>
      <div className="space-y-3">
        {parcours.missions.map((mission) => (
          <MissionCard key={mission.id} mission={mission} progress={progress}
            onToggleArticle={onToggleArticle} onSubmitQuiz={onSubmitQuiz} onResetQuiz={onResetQuiz} />
        ))}
      </div>
    </section>
  );
...
const LEVELS = [
  ...
  { name: "Maître·sse", emoji: "🏆", min: 99999 },
];

// ─── Main ─────────────────────────────────────────────────────────────────────

export default function AcademieRoleClient({ roleId, userEmail: initialUserEmail }: { roleId: string; userEmail: string | null }) {
  ...
  useEffect(() => {
    const loadAll = async () => {
      ...
      const captured = localStorage.getItem(LEAD_KEY) === "1";
      setLeadCaptured(captured);

      // Try to merge with remote progress if authenticated
  ...
  const persistProgress = useCallback(async (p: Progress) => {
    ...
    saveRemoteProgress({
      completed_articles: [...p.completedArticles],
      unlocked_badges: [...p.unlockedBadges],
      quiz_results: p.quizResults,
      earned_quiz_xp: p.earnedQuizXp,
    });
  }, []);
  ...
  const checkBadges = useCallback((p: Progress) => {
    ...
    for (const parcours of role.parcours) {
      const allDone = parcours.missions.every((m) =>
        m.articles.every((a) => updated.completedArticles.has(a.id)) && updated.quizResults[m.id]?.submitted
      );
  ...
  const handleToggleArticle = useCallback((articleId: string) => {
    setProgress((prev) => {
      ...
      if (next.completedArticles.has(articleId)) {
        ...
        showToast("xp", "+XP · Contenu validé 🎉");
        // Trigger lead modal after threshold
        if (!leadCaptured && !userEmail && next.completedArticles.size === LEAD_TRIGGER_COUNT) {
          setTimeout(() => setShowLeadModal(true), 600);
        }
      }
  ...
  }, [checkBadges, persistProgress, showToast, leadCaptured, userEmail]);
  ...
  const handleSubmitQuiz = useCallback((missionId: string, answers: Record<string, number>) => {
    setProgress((prev) => {
      ...
      const checked = checkBadges(next);
      showToast("xp", `+${xpEarned} XP · ${score === mission.quiz.length ? "Score parfait ! 🏆" : `${score}/${mission.quiz.length} bonnes réponses`}`);
      persistProgress(checked);
      return checked;
    });
  }, [role, checkBadges, persistProgress, showToast]);

  const handleResetQuiz = useCallback((missionId: string) => {
    setProgress((prev) => {
      const next = { ...prev, quizResults: { ...prev.quizResults, [missionId]: { answers: {}, submitted: false } }, earnedQuizXp: { ...prev.earnedQuizXp, [missionId]: 0 } };
      saveLocalProgress(next);
      return next;
    });
  }, []);

  function handleLeadClose(captured: boolean) {
  ...
  const allDone = globalPct === 100 && role.parcours.flatMap((p) => p.missions).every((m) => progress.quizResults[m.id]?.submitted);

  return (
    <div className="min-h-screen bg-[#F8F9FF]">
      {/* Lead gen modal */}
      {showLeadModal && <LeadModal role={roleId} onClose={handleLeadClose} />}

      {/* Toast */}
      {toast && (
        <div className={`fixed right-6 top-6 z-40 slide-in-right rounded-2xl px-5 py-4 shadow-2xl ${toast.type === "badge" ? "bg-gradient-to-r from-amber-400 to-yellow-400 shadow-amber-200" : "asso-gradient shadow-[#3D5AF1]/30"}`}>
      ...
      <div className="relative overflow-hidden bg-white border-b border-[#E8EEFF]">
        ...
        <div className="relative mx-auto max-w-3xl px-6 py-8">
          <Link href="/academie" className="mb-5 inline-flex items-center gap-2 rounded-full border border-[#E8EEFF] bg-[#F5F7FF] px-4 py-1.5 text-sm font-semibold text-[#3D5AF1] hover:bg-[#E8EEFF] transition-colors">
            ← Changer de rôle
          </Link>

          <div className="flex items-start justify-between gap-6 mb-5">
            <div className="flex items-center gap-4">
              <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#E8EEFF] text-4xl shadow-lg shadow-[#3D5AF1]/10">{role.emoji}</div>
              <div>
          ...
          {/* SSO bar */}
          {mounted && <div className="mb-5"><SSOBar userEmail={userEmail} onAuthChange={handleAuthChange} /></div>}
          ...
          <div>
            ...
            <div className="h-3 overflow-hidden rounded-full bg-[#E8EEFF]">
              <div className="h-full rounded-full asso-gradient transition-all duration-700" style={{ width: `${mounted ? globalPct : 0}%` }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-3xl space-y-6 px-6 py-10">
        {role.parcours.map((parcours) => (
          <ParcoursSection key={parcours.id} parcours={parcours} progress={progress}
            onToggleArticle={handleToggleArticle} onSubmitQuiz={handleSubmitQuiz} onResetQuiz={handleResetQuiz} />
        ))}
