...
function SidebarContent({ reportId, pathname, report, orgName, onClose }: {
  reportId: string
  pathname: string
  report: Report | null
  orgName: string
      ...
      <div className="bg-gradient-to-br from-indigo-600 to-violet-600 px-4 py-5">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-bold text-sm truncate">{orgName}</span>
          </div>
          {onClose && (
...
export default function ReportIdLayout({ children, orgName }: { children: React.ReactNode; orgName: string }) {
    ...
    <div className="flex gap-6 relative">
      ...
      <aside className="w-56 shrink-0 hidden md:block">
        <div className="rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-6">
          <SidebarContent reportId={id} pathname={pathname} report={report} orgName={orgName} />
        </div>
      </aside>
